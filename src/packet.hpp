...
#define GET_SHORT(pack)   ntohs(*(short*)pack);  pack+=sizeof(short);
#define GET_INT(pack)     ntohl(*(int*)pack);    pack+=sizeof(int);
...
#define SET_SHORT(pack,v)   *(short*)pack=htons(v);  pack+=sizeof(short);
#define SET_INT(pack,v)     *(int*)pack=htonl(v);    pack+=sizeof(int);
